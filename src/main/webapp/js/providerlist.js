...
function deleteProvider(obj){
	$.ajax({
		...
		success:function(data){
			...
			if(data.num>0){
				changeDLGContent("对不起，该供应商【"+obj.attr("proname")+"】下有【"+data.delResult+"】条订单，不能删除");
			}else if(data.success){//删除成功：移除删除行
				cancleBtn();
				obj.parents("tr").remove();
			}else {
				changeDLGContent("对不起，删除供应商【"+obj.attr("proname")+"】失败");
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/viewProvider?method=view&proid="+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/updateViewProvider?method=modify&proid="+ obj.attr("proid");
	});
